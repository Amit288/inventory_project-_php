...
class Manage
{
	...
	public function joinInvoice($table,$table1,$from_date,$to_date){

		
		$query = "SELECT {$table}.order_date,{$table}.invoice_no,{$table}.customer_name,{$table1}.product_name,{$table1}.qty,{$table}.net_total,{$table}.paid,{$table}.due,{$table}.payment_type FROM {$table} LEFT JOIN {$table1} ON {$table}.invoice_no={$table1}.invoice_no WHERE {$table}.order_date BETWEEN '{$from_date}' AND '{$to_date}'";
		$result = $this->con->query($query) or die($this->con->error);
		
		return $result;
	}
	
	// Invoice report without date

	public function showInvoice($table,$table1){
